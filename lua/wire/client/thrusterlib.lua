...
WireLib.ThrusterEffectThink.smoke_diy = smoke(function(self) local color = self:GetColor() return color.r, color.g, color.b end)
...
WireLib.ThrusterEffectThink.magic_diy = magic(function(self) local color = self:GetColor() return color.r, color.g, color.b end)
WireLib.ThrusterEffectThink.magic_color = magic(function() return math.random(0,255), math.random(0,255), math.random(0,255) end)

local function squirt(effect, delay, scale)
	return function(self)
		self.RingTimer = self.RingTimer or 0
		if ( self.RingTimer > CurTime() ) then return end
...
WireLib.ThrusterEffectDraw.rings_grow = squirt("thruster_ring_grow", 0.00005, 1)
...
WireLib.ThrusterEffectDraw.rings_shrink = squirt("thruster_ring_shrink", 0.00005, 1)
